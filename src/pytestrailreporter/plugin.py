...
from .testrail_client import configure_testrail_client, add_testrail_run
from .result_sender import send_testrail_results, result_queue
...
def pytest_addoption(parser):
    ...
    group.addoption(
        "--testrail-url", action="store", help="TestRail URL", default=os.environ.get("TESTRAIL_URL"))
    group.addoption(
        "--testrail-user", action="store", help="TestRail user", default=os.environ.get("TESTRAIL_USER"))
    group.addoption(
        "--testrail-password", action="store", help="TestRail password", default=os.environ.get("TESTRAIL_PASSWORD"))
    group.addoption(
        "--testrail-project-id", type=int, help="TestRail project ID", default=os.environ.get("TESTRAIL_PROJECT_ID"))
    group.addoption(
        "--testrail-run-name", help="TestRail run name", default=os.environ.get("TESTRAIL_RUN_NAME"))
    group.addoption(
        "--testrail-config", help="Path to TestRail config YAML")

def pytest_configure(config):
    """Configure TestRail integration."""
    config.addinivalue_line(
        "markers", "testrail_cases(cases): Mark test with TestRail case IDs.")
    ...
    try:
        ...
        config.testrail_run_id = add_testrail_run(
            config.testrail_client, testrail_config["project_id"], testrail_config["run_name"]
        )
        logging.info(f"TestRail run created: {config.testrail_run_id}")
        threading.Thread(target=send_testrail_results, args=(config.testrail_client, config.testrail_run_id), daemon=True).start()
